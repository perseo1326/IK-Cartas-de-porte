...
const uploadFileOverviewButton = document.getElementById(
    "upload-file-b-overview"
);
const fileSelectorHistorical = document.getElementById("file-input-historical");
const uploadFileHistorical = document.getElementById(
    "upload-file-b-historical"
);
const fileSelectorByStatus = document.getElementById(
    "file-input-by-order-status"
);
const uploadFileByStatus = document.getElementById(
    "upload-file-b-by-order-status"
);
...
// variable for contain all the JSON configuration data 
// for the selected shop
...
shopSeleccionB.addEventListener("click", loadConfigurationPUP);

// blocking the mouse wheel to avoid scroll the panel and visualization
panelShippingDate.addEventListener("wheel", (evento) => {
    evento.preventDefault();
});
waitPanel.addEventListener("wheel", (evento) => {
    evento.preventDefault();
});
...
function loadConfigurationPUP( ) {
    ...
    remoteResourcePromise.then( (resource) => {
        ...
        if(typeof(configData) === undefined || configData === 0 ) {
            ...
            if(configData.shopCode !== Number( SHOP_CODE_SELECTED ) ) {
        ...
        shopSelectorPanel.style.display = "none";
        })
...
// *********************************************************
// function to retrieve data from an external source
async function getRemoteResource ( URL ){
    console.log("INFO:getRemoteResource: SOURCE URL:\n", URL );
    // set up wait panel
    waitPanel.style.display = "block";

    let remoteResourceData = 0;
    const options = {
        method: 'GET',
        headers: {
    ...
    await fetch( URL, options )
        .then((response) => {
            if (!response.ok) {
                throw new Error("Problema al obtener configuración remota.");
            }
            return response.json();
        })
        .then((userData) => {
            // Process the retrieved user data
            remoteResourceData = userData;
        })
        .catch((error) => {
            // console.error("Error:", error);
            // throw new Error("ERROR EN PROMISE RESOURCE");
        })
        .finally( () => {
            waitPanel.style.display = "none";
            console.log("Finalizando fetch, DATA: \n", remoteResourceData);
        });
    ...
    try {
        ...
        switch (evento.target.id) {
            ...
            case "file-input-overview":
                ...
                uploadFileOverviewButton.innerText =
                    fileCSVOverview.file.name +
                    " (" +
                    fileDate.getHours() +
                    ":" +
                    fileDate.getMinutes() +
                ...
                dataPromise
                    .then((response) => {
                        ...
                        let arrayDataClean = readOverviewFileCSV(
                            fileCSVOverview.file,
                            response
                        );
                        isellsOverviewMapComplet = mappingArrayDataCSV(arrayDataClean);
                        showSelectionButton();
                        console.log(
                            'Carga Overview "' + fileCSVOverview.file.name + '" Finalizada!'
                        );
                        waitPanel.style.display = "none";
            ...
            case "file-input-historical":
                ...
                uploadFileHistorical.innerText =
                    fileHistorical.file.name +
                    " (" +
                    fileDate.getHours() +
                    ":" +
                    fileDate.getMinutes() +
...
function loadReportsExcel(excelFile, button) {
    ...
    fileReader.onload = function () {
        try {
            ...
            let contentFile = XLSX.utils.sheet_to_row_object_array(
                workbook.Sheets[WORKING_SHEET]
            );

            // process and clean info from the file
            let arrayExcelClean = readReportsExcel(excelFile.file, contentFile);
            console.log(
                'Carga "' + excelFile.file.name + '" Finalizada!',
                arrayExcelClean
            );
...
function processData() {
    try {
        ...
        if (!cutOffTimeObj) {
            console.log(
                "WARNING:processData: No se ha seleccionado un destino (CUT OFF TIME)"
            );
            throw new Error("No se ha seleccionado un destino (CUT OFF TIME)");
        }

        // Find the object selected in the "SERVICE_WINDOW" selector
        const windowServiceObject = cutOffTimeObj.windowService.find((service) => {
            return service.serviceCode === serviceWindowSelector.value;
        });
        if (!windowServiceObject) {
            console.log(
                'WARNING:processData: No se ha seleccionado un "Service Window" válido.'
            );
        ...
        ordersMap = dataFilterByServiceWindow(
            ordersMap,
        ...
        frameDestination.innerText =
            cutOffTimeObj.title + " - " + windowServiceObject.serviceName;

        setAddressTransportDocument(
            document.getElementById("sender-address"),
            cutOffTimeObj.senderAddress
        );
        setAddressTransportDocument(
            document.getElementById("consignee-address"),
            cutOffTimeObj.consigneeAddress
        );
        setAddressTransportDocument(
            document.getElementById("carrier-address"),
            cutOffTimeObj.carrierAddress
        );
        ...
        printDocumentTitle =
            windowServiceObject.serviceNameShort + "_" + cutOffTimeObj.title;
    } catch (error) {
        console.log("ERROR:", error);
        alert(error.message);
    }
...
function combineOrdersWithDetails(orders, detailsMap) {
    orders.forEach((order, isell) => {
        if (
            ...
            detailsMap.isellsMap.has(order[ISELL_ORDER])
        ) {
